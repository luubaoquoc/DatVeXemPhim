...
const DienVien = () => {
  ...
  const fetchDienViens = async () => {
    try {
      const response = await api.get('/dienvien',
        { params: { page: currentPage, limit, search } }
      )
      setDienViens(response.data.data)
      ...
      <div className="flex flex-col lg:flex-row gap-10 max-w-7xl mx-auto">
        ...
        <div className="flex-2">
          ...
          <div className="flex flex-col gap-3">
            {dienViens.map((dienVien) => (
              <div key={dienVien.maDienVien} className="bg-black/30 rounded-lg overflow-hidden shadow-lg flex gap-3">
                <img
                  src={dienVien.anhDaiDien || "https://i.pinimg.com/736x/bc/43/98/bc439871417621836a0eeea768d60944.jpg"}
                  alt={dienVien.tenDienVien}
                  className="w-48 h-48 object-cover rounded-lg"
                />
                <div className="p-4">
                  <h3 className="text-xl font-semibold mb-2">{dienVien.tenDienVien}</h3>
                  <p className='mb-2'><span className='text-primary font-medium'>Ngày sinh: </span> {dienVien.ngaySinh ? formatDate(dienVien.ngaySinh) : "Chưa cập nhật"}</p>
                  <p className="text-gray-400"><span className='text-primary'>Tiểu sử: </span>{dienVien.tieuSu ? dienVien.tieuSu : 'Chưa cập nhật'}</p>
                </div>
              </div>
            ))}
          </div>
