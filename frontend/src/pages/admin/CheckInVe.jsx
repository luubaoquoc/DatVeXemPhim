...
const CheckInVe = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await api.get(`/datve/${maDatVe}/checkin`);
      setVe(res.data);
  ...
  const handleCheckIn = async () => {
    try {
      const res = await api.post(`/datve/${maDatVe}/checkin`);
  ...
  const ghe = ve?.chiTietDatVes
    ?.map((ct) => `${ct.ghe?.hang}${ct.ghe?.soGhe}`)
    .join(", ");

  return (
    <div className="p-6 max-w-xl">
      <h1 className="text-3xl font-bold mb-12 ">Quét & Check-in Vé</h1>
          ...
          <div className="bg-black/80 rounded-2xl shadow-2xl w-120 p-6 border border-primary relative">
            ...
            <div className="space-y-3 text-gray-700">
              <p className="flex items-center gap-2 text-white">
                ...
                <b>Mã đặt vé:</b> {ve.maDatVe}
              </p>

              <p className="flex items-center gap-2 text-white">
                <Film size={18} className="text-red-600" />
                <b>Tên phim:</b> {ve.suatChieu?.phim?.tenPhim}
              </p>

              <p className="flex items-center gap-2 text-white">
                <Clock size={18} className="text-orange-600" />
                <b>Suất chiếu:</b> {ve.suatChieu?.gioBatDau}
              </p>
